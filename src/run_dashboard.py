...
def calculate_status(
    ...
    postchecks = load_latest_postchecks(postcheck_journal_paths(run_dir))
...
<body><main>
  <header>
    <div><h1>SWE-gen pipeline</h1><div class="sub">Run <span id="run">—</span> · exact evidence, two stages</div></div>
  ...
  <section class="panel pipeline">
    <article class="stage">
      <div class="stage-header">
        <div><div class="stage-number">Stage I</div><h2>SWE-gen</h2><div class="sub">Generation plus NOP and Oracle validation</div></div>
        <div class="stage-total"><div id="stageOneTotal" class="count">— / —</div><div id="stageOneEquation" class="equation">—</div></div>
      </div>
      <div id="stageOneBar" class="stacked-bar" role="img" aria-label="Stage I counts loading">
        <div id="stageOneGreenBar" class="segment green"></div><div id="stageOneRedBar" class="segment red"></div><div id="stageOneGrayBar" class="segment gray"></div>
      </div>
      ...
      <div class="legend">
        <div class="legend-item"><span class="swatch green"></span><div><div class="legend-label">Valid</div><div id="stageOneGreenValue" class="legend-value">—</div><div class="legend-detail">NOP=0 and Oracle=1</div></div></div>
        <div class="legend-item"><span class="swatch red"></span><div><div class="legend-label">Oracle rejected</div><div id="stageOneRedValue" class="legend-value">—</div><div class="legend-detail">Explicit Oracle=0 only</div></div></div>
        <div class="legend-item"><span class="swatch gray"></span><div><div class="legend-label">Unprocessed / unresolved</div><div id="stageOneGrayValue" class="legend-value">—</div><div class="legend-detail">No definitive Oracle result</div></div></div>
      </div>
      <div id="stageOneMeta" class="meta">—</div>
    </article>

    <article class="stage">
      <div class="stage-header">
        <div><div class="stage-number">Stage II</div><h2>Reward-hack filter</h2><div class="sub">Runs only on Stage I green tasks</div></div>
        <div class="stage-total"><div id="stageTwoTotal" class="count">— / —</div><div id="stageTwoEquation" class="equation">—</div></div>
      </div>
      <div id="stageTwoBar" class="stacked-bar" role="img" aria-label="Stage II counts loading">
        <div id="stageTwoGreenBar" class="segment green"></div><div id="stageTwoRedBar" class="segment red"></div><div id="stageTwoGrayBar" class="segment gray"></div>
      </div>
      <div class="legend">
        <div class="legend-item"><span class="swatch green"></span><div><div class="legend-label">Accepted</div><div id="stageTwoGreenValue" class="legend-value">—</div><div class="legend-detail">Reward-hack clean</div></div></div>
        <div class="legend-item"><span class="swatch red"></span><div><div class="legend-label">Filtered</div><div id="stageTwoRedValue" class="legend-value">—</div><div class="legend-detail">Reward hacking detected</div></div></div>
        <div class="legend-item"><span class="swatch gray"></span><div><div class="legend-label">Unprocessed / retryable</div><div id="stageTwoGrayValue" class="legend-value">—</div><div class="legend-detail">Pending, running, or API error</div></div></div>
      </div>
      <div id="stageTwoMeta" class="meta">—</div>
    </article>
  </section>

  <section class="health">
    <div class="panel health-card"><div class="health-name">Stage I runtime</div><div id="stageOneHealth" class="health-value">—</div></div>
    <div class="panel health-card"><div class="health-name">Stage II runtime</div><div id="stageTwoHealth" class="health-value">—</div></div>
  </section>
  <footer aria-live="polite">Updated <span id="updated">—</span> · refreshes every 5 seconds</footer>
</main>
<script>
...
async function refresh() {
  try {
    ...
    const data=await response.json(); const pipeline=data.pipeline||{}; const stageOne=pipeline.stage_i||{}; const stageTwo=pipeline.stage_ii||{}; const health=data.health||{};
    text('run',data.run||'—');
    renderStage('stageOne',stageOne,{stage:'Stage I SWE-gen',green:'valid',red:'Oracle rejected',gray:'unprocessed or unresolved'});
    renderThroughput(data.stage_i_throughput||{});
    renderStage('stageTwo',stageTwo,{stage:'Stage II reward-hack filter',green:'accepted',red:'filtered',gray:'unprocessed or retryable'});
    const first=stageOne.breakdown||{}; const recovered=stageOne.red_breakdown||{}; text('stageOneMeta',`${fmt(first.unprocessed)} unprocessed · ${fmt(first.validation_unresolved)} unresolved NOP/Oracle failures · ${fmt(first.transient_api)} transient API errors · ${fmt(first.incomplete_or_other)} incomplete/other · ${fmt(recovered.historical_recovered)} Oracle=0 recovered from historical Harbor runs`);
    const second=stageTwo.breakdown||{}; text('stageTwoMeta',`${fmt(second.pending)} pending · ${fmt(second.running)} running · ${fmt(second.errors)} retryable/API errors`);
    const generation=health.generation||{}; const baseline=health.baseline||{}; const reward=health.reward||{};
    const generationState=generation.state||'unavailable'; const baselineState=baseline.state||'not_running';
    const baselineCapacity=baseline.baseline_concurrency ?? 1; const baselineActive=baseline.baseline_active_count ?? 0;
    text('stageOneHealth',`${healthLabel(generationState)} collector · ${fmt(generation.active_workers)} active generators · NOP/Oracle ${fmt(baselineActive)}/${fmt(baselineCapacity)} active · ${healthLabel(baselineState).toLowerCase()}`);
    document.getElementById('stageOneHealth').className=`health-value ${stateClass(generationState==='error'||baselineState==='error'?'error':generationState)}`;
    const rewardState=reward.state||'not_running'; const fallback=reward.reward_fallback_model?` → ${reward.reward_fallback_model}`:''; const model=reward.reward_primary_model?` · ${reward.reward_primary_model}${fallback}`:'';
    text('stageTwoHealth',`${healthLabel(rewardState)} · ${fmt(reward.reward_active_count)}/${fmt(reward.reward_concurrency)} active${model}`);
    document.getElementById('stageTwoHealth').className=`health-value ${stateClass(rewardState)}`;
