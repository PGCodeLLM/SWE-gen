...
def load_runtime_credentials() -> tuple[dict[str, str], str]:
    ...
    # Required API-credential keys.
    _credential_keys = (
        "OPENAI_API_KEY",
        "ANTHROPIC_API_KEY",
        "ANTHROPIC_AUTH_TOKEN",
    )
    # Optional endpoint/model overrides forwarded into the node bundle's backend
    # profile so preflight and run_orchestrator.sh pick up a non-default agent
    # backend (e.g. glm-5.2-moedsa at a private endpoint) instead of the tailnet
    # defaults baked into run_orchestrator.sh.  Only carried when actually set,
    # so an empty value never clobbers run_orchestrator.sh's ${VAR:-default}.
    _endpoint_model_keys = (
        "OPENAI_BASE_URL",
        "ANTHROPIC_BASE_URL",
        "OPENAI_MODEL",
        "ANTHROPIC_MODEL",
    ...
    def selected(values: dict[str, str]) -> dict[str, str]:
        result = {key: values.get(key, "") for key in _credential_keys}
        if not result["ANTHROPIC_API_KEY"]:
            result["ANTHROPIC_API_KEY"] = result["ANTHROPIC_AUTH_TOKEN"]
        if not result["ANTHROPIC_AUTH_TOKEN"]:
            result["ANTHROPIC_AUTH_TOKEN"] = result["OPENAI_API_KEY"]
        for key in _endpoint_model_keys:
    ...
    if all(current.get(key) for key in _credential_keys):
    ...
    for entry in Path("/proc").iterdir():
        ...
        for item in raw_environment:
            ...
            if name in _credential_keys or name in _endpoint_model_keys:
                values[name] = value.decode(errors="replace")
        credentials = selected(values)
        if all(credentials.get(key) for key in _credential_keys):
