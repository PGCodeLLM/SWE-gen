...
# Server-side cadence for the whole kubectl/Postgres fan-out, and the browser's
# matching poll cadence. Both moved 5s -> 15s: the dashboard's 5s refresh was
# enough, on its own, to take the k3s API server from passing 4/4 /livez probes
# to failing 3/3 against ~2,400 pods. The browser value is injected into the
# page so the two cannot drift apart from one env var.
DASHBOARD_REFRESH_SECONDS_ENV = "SWEGEN_DASHBOARD_REFRESH_SECONDS"
DASHBOARD_REFRESH_SECONDS_DEFAULT = 15.0

SCALE_MIN = 0
BUILD_SLOT_MIN = 1
# Upper bound on rows a single bulk concurrency apply may carry. The registry
# holds a handful of endpoints; this caps the work one request can do and keeps
# a well-formed batch inside the POST body limit.
MAX_BULK_SCALE_ITEMS = 50
# Bulk scale carries one {slug, concurrency} object per row, so its body is
# legitimately larger than the single-action 1 KiB ceiling every other control
# route lives under. Sized for MAX_BULK_SCALE_ITEMS rows of a 41-char slug.
MAX_CONTROL_BODY_BYTES = 1024
MAX_BULK_SCALE_BODY_BYTES = 8192
...
class GenerateEndpointRegistry:
    ...
    def scale_many(self, *, items: object, max_replicas: object) -> dict[str, Any]:
        """Apply a batch of per-endpoint concurrency targets in one request.

        The inline spinners in the endpoints table let an operator dial several
        rows and commit them with one Apply, so this is the bulk form of
        ``scale``. It deliberately runs in two phases:

        Phase 1 validates EVERY item -- shape, slug format, and concurrency
        bounds -- before a single UPDATE runs, reusing ``_validate_concurrency``
        so the bounds are byte-identical to the single-row path. Any invalid
        item aborts the whole batch with nothing written, so a typo in one
        spinner can never half-apply the others. Duplicate slugs are rejected
        here too: two entries for one endpoint would otherwise silently
        last-write-wins.

        Phase 2 applies the validated rows. A row whose slug vanished between
        the operator's page load and the Apply (concurrent delete) is recorded
        in ``failed`` and the remaining rows still apply -- but the response
        reports both lists explicitly and ``ok`` is False, so the UI can tell
        the operator exactly which endpoints took and which did not. An
        operator must never believe they set five rows when only three landed.

        Rows whose breaker is latched are accepted: ``concurrency`` is the
        stored TARGET the controller will honour once Reset clears the latch,
        not an immediate scale, and refusing the write would force the operator
        to unlatch at a stale target before they could correct it.
        """

        if not isinstance(items, list) or not items:
            raise ValueError("items must be a non-empty list")
        if len(items) > MAX_BULK_SCALE_ITEMS:
            raise ValueError(f"at most {MAX_BULK_SCALE_ITEMS} endpoints per apply")
        # Phase 1: validate everything up front; no write happens unless the
        # entire batch is well-formed and in range.
        planned: list[tuple[str, int]] = []
        seen: set[str] = set()
        for item in items:
            if not isinstance(item, dict):
                raise ValueError("each item must be an object with slug and concurrency")
            target = self._require_slug(item.get("slug"))
            if target in seen:
                raise ValueError(f"duplicate slug in request: {target}")
            seen.add(target)
            planned.append(
                (target, _validate_concurrency(item.get("concurrency"), max_replicas=max_replicas))
            )
        # Phase 2: apply. Every row is attempted so the report is complete.
        applied: list[dict[str, Any]] = []
        failed: list[dict[str, Any]] = []
        with self._lock, self._connection() as conn:
            for target, count in planned:
                model = self._model_for(conn, target)
                result = conn.execute(
                    "UPDATE generate_endpoints SET concurrency = %s, updated_at = now() "
                    "WHERE slug = %s",
                ...
                if getattr(result, "rowcount", 0) != 1:
                    failed.append({"slug": target, "error": "endpoint not found"})
                    continue
                self._event(conn, target, model, "scaled", f"concurrency set to {count}")
                applied.append({"slug": target, "concurrency": count})
...
class SnapshotCache:
    def __init__(self, *, refresh_seconds: float | None = None) -> None:
        # Every refresh re-issues the whole kubectl fan-out against an API server
        # that is paging ~2,400 pods out of etcd. At 5s this was enough on its
        # own to push k3s into failing its /livez probes; the server recovered to
        # 4/4 passing within 30s of stopping the dashboard. 15s is the default,
        # overridable per deployment.
        self.refresh_seconds = (
            refresh_seconds
            if refresh_seconds is not None
            else resolve_positive_env_float(
                DASHBOARD_REFRESH_SECONDS_ENV, DASHBOARD_REFRESH_SECONDS_DEFAULT
            )
        )
        self._lock = threading.Lock()
...
<!-- Parsed here so its ids/listeners exist at boot, then MOVED (not cloned) into
the generate stage card by stageCard(); until the first poll lands it stays
visible right under the empty stages container, so the form works even if the
status API never answers. -->
<div id="endpoints-panel" class="endpoints-panel">
<div class="endpoints-panel-title">Generate model endpoints</div><div class="muted endpoints-panel-note">Each registered endpoint runs its own generate worker pool at its own concurrency. A controller reconciles the pods and latches a breaker on repeated endpoint 5xx/429; the token is stored server-side and never shown.</div>
...
<div id="endpoint-apply-bar" class="endpoint-apply-bar"><button id="endpoint-apply" type="button" disabled>Apply concurrency changes</button><span id="endpoint-apply-note" class="muted endpoint-apply-note"></span></div>
</div>
...
/* A relative age from a NUMBER of seconds (the payload's disk_io.age_seconds),
not from a timestamp: the sample's age is measured server-side on a monotonic
clock, so it stays right even if the browser's wall clock disagrees. */
const ageLabel=s=>!Number.isFinite(s)?'unknown age':s<60?agoUnit(Math.round(s),'second'):s<3600?agoUnit(Math.round(s/60),'minute'):agoUnit(Math.round(s/3600),'hour');
/* Disk I/O is the one panel fed by the node cAdvisor endpoint, which at ~600
pods per node can stop answering entirely. Its per-node breaker is surfaced here
so an operator reads "disk IO unavailable for node X" or "disk IO from 4m ago"
instead of silently trusting numbers that stopped updating. */
function diskIoLabel(io){if(!io)return {text:formatDiskIo(io),detail:'disk I/O sample unavailable',bad:false};const breaker=io.breaker||{};if(!io.available){const why=breaker.open?`polling suspended after ${breaker.consecutive_failures} consecutive failures${breaker.cooldown_remaining_seconds==null?'':`; retrying in ${secs(breaker.cooldown_remaining_seconds)}`}`:friendlyError(io.error)||'no sample yet';return {text:`disk I/O unavailable — ${why}`,detail:friendlyError(io.error||breaker.reason||''),bad:breaker.open}}const suffix=io.stale?` · from ${ageLabel(io.age_seconds)}`:'';const detail=io.stale?`Last successful cAdvisor sample, ${ageLabel(io.age_seconds)}.${breaker.open?` Polling suspended after ${breaker.consecutive_failures} consecutive failures${breaker.cooldown_remaining_seconds==null?'':`; retrying in ${secs(breaker.cooldown_remaining_seconds)}`}.`:''}${io.error?` ${friendlyError(io.error)}`:''}`:`Live cAdvisor rate sample over ${secs(io.sample_window_seconds)}.`;return {text:`${formatDiskIo(io)}${suffix}`,detail,bad:io.stale}}
...
const RANGE_HOURS_ALLOWED=[24,72,168];const uiState={chartScroll:{},expandedTasks:new Set(),expandedNodes:new Set(),resourceScroll:{left:0,top:0},scaleDrafts:{},scaling:false,buildSlotDrafts:{},buildSlotUpdating:false,endpointBusy:false,endpointConcDrafts:{},rangeHours:24,modelColors:{},stageCards:null,polling:false,lastPollMs:null};
...
/* offsetLeft is measured from the nearest POSITIONED ancestor, and nothing
between .bucket and <body> sets position, so a cell's offsetLeft is a PAGE
coordinate that includes wherever the stage card happens to sit in the layout
(~600px in for the generate card). chart.scrollLeft is a CONTENT coordinate
starting at 0. Comparing the two mixed the frames: the "visible" set was shifted
by the card's page offset, so on first paint (scrolled to the newest data) the
scope resolved to the OLDEST buckets. That is why a 213 bucket rendered as tall
as a 572 one -- 213 was the max of that wrongly-chosen scope, and the genuinely
tall recent bars were never in scope to be rescaled. Subtracting the chart's own
offsetLeft converts the cell back into the chart's content frame -- valid because
a .bucket's offsetParent IS the chart's offsetParent (neither .chart nor .bucket
is positioned), so both are measured from the same origin and the difference is
exact. Guarded by making the chart itself the positioning context in CSS, which
pins offsetParent to .chart and makes originLeft 0; the subtraction then costs
nothing and still holds if that CSS is ever removed. */
function rescaleVisible(chart){const cells=chart._buckets;if(!cells||!cells.length)return;const viewLeft=chart.scrollLeft||0,viewRight=viewLeft+(chart.clientWidth||0);const originLeft=chart.offsetLeft||0;const visible=cells.filter(c=>{const left=(c.cell.offsetLeft||0)-originLeft,width=c.cell.offsetWidth||0;return width<=0?true:(left<viewRight&&left+width>viewLeft)});const scope=visible.length?visible:cells;const visibleUpMax=Math.max(1,...scope.map(c=>c.up));const visibleDownMax=Math.max(1,...scope.map(c=>c.down));/* Heights are applied to the SCOPE, not to every cell. Applying a visible-window denominator to off-screen cells was the clipping bug: a bucket taller than the window max got Math.min(100,...) and pinned flat, so e.g. a 572 bucket rendered identically to the 213 that happened to be on screen. Off-screen cells are left alone and recomputed by the scroll handler when they come into view. */scope.forEach(c=>{applyBucketHeights(c,visibleUpMax,visibleDownMax)});const yAxis=chart._yAxis;if(yAxis){const ticks=yAxis.children;[visibleUpMax,0,-visibleDownMax].forEach((value,index)=>{const tick=ticks[index];if(tick)setText(tick,value<0?`-${compactChartCount(-value)}`:compactChartCount(value))})}}
/* Single writer for a bucket's bar geometry, shared by the initial render and
every rescale so the two can never disagree about how a value maps to a height.

A bar is drawn as value/max, and a value ABOVE the current max is clamped to
100% -- but a clamped bar is also marked `.clipped`, which paints a hatched cap
on it. That is the honesty requirement: a bar may never silently render at a
height that misrepresents its value. Within a correctly-scoped rescale nothing
should clip (the denominator is the scope max), so the marker is a visible
assertion that the scale is right, and a real affordance in the one case where
clipping is legitimate -- the brief moment before a newly-scrolled-in cell is
recomputed. Segments are proportional to their OWN bucket total, so they are
unaffected by the scale and always sum to their bar. */
function applyBucketHeights(c,upMax,downMax){const upPercent=c.up/(upMax||1)*100,downPercent=c.down/(downMax||1)*100;c.upBar.style.height=`${Math.min(100,upPercent)}%`;c.downBar.style.height=`${Math.min(100,downPercent)}%`;if(c.upBar.classList)c.upBar.classList.toggle('clipped',upPercent>100.0001);if(c.downBar.classList)c.downBar.classList.toggle('clipped',downPercent>100.0001);c.upSegs.forEach(s=>{s.seg.style.height=`${c.up?s.value/c.up*100:0}%`});c.downSegs.forEach(s=>{s.seg.style.height=`${c.down?s.value/c.down*100:0}%`})}
...
/* The bulk Apply button is disabled while busy like every other endpoint action,
but it must NOT be re-enabled blindly when the request settles: it stays disabled
unless there is still a valid pending edit. renderEndpointApplyBar owns that
decision, so it runs after the generic sweep. */
function renderEndpointButtonsDisabled(){document.querySelectorAll('#endpoint-form button,.endpoint-actions button,#endpoint-apply').forEach(button=>{button.disabled=uiState.endpointBusy});const applyBtn=el('endpoint-apply');if(applyBtn&&!uiState.endpointBusy&&applyBtn.dataset.hasPending!=='true')applyBtn.disabled=true}
...
/* Inline concurrency spinners. Each row's number input writes its raw string into
uiState.endpointConcDrafts[slug] on every keystroke; a draft is DELETED once it
matches the server value again, so "dirty" is simply "has a draft that differs
from the row's stored concurrency". Keeping drafts keyed by slug (not by DOM
position) is what lets the 5s poll re-render the table underneath the operator
without discarding half-typed edits. */
function endpointConcDraft(slug,stored){const draft=uiState.endpointConcDrafts[slug];return draft===undefined?String(stored??0):draft}
function endpointDirtyRows(endpoints){return (endpoints||[]).filter(ep=>{const draft=uiState.endpointConcDrafts[ep.slug];return draft!==undefined&&draft!==String(ep.concurrency??0)})}
/* Enable Apply only when at least one row is dirty AND every dirty row holds a
whole number within [0,maxReplicas] -- the same bounds the server enforces, so
the button never invites a request the backend must reject. */
function endpointApplyState(endpoints,maxReplicas){const dirty=endpointDirtyRows(endpoints);const capacityAvailable=Number.isSafeInteger(maxReplicas)&&maxReplicas>=0;const invalid=dirty.filter(ep=>{const raw=uiState.endpointConcDrafts[ep.slug];if(!/^\d+$/.test(String(raw).trim()))return true;const value=Number(raw);return !Number.isSafeInteger(value)||value<0||(capacityAvailable&&value>maxReplicas)});return {dirty,invalid,capacityAvailable,enabled:Boolean(dirty.length)&&!invalid.length&&capacityAvailable&&!uiState.endpointBusy}}
function renderEndpointApplyBar(endpoints,maxReplicas){const bar=el('endpoint-apply-bar');if(!bar)return;const state=endpointApplyState(endpoints,maxReplicas);const button=el('endpoint-apply');const note=el('endpoint-apply-note');if(button){button.dataset.hasPending=String(Boolean(state.dirty.length)&&!state.invalid.length&&state.capacityAvailable);button.disabled=!state.enabled}if(note){note.className=state.invalid.length?'bad endpoint-apply-note':'muted endpoint-apply-note';if(!state.capacityAvailable)setText(note,'Cluster scaling capacity is unavailable; concurrency cannot be applied.');else if(state.invalid.length)setText(note,`Fix ${state.invalid.length} invalid value(s): whole numbers from 0 to ${maxReplicas}.`);else if(!state.dirty.length)setText(note,`No pending concurrency changes · ceiling ${maxReplicas}`);else setText(note,`${state.dirty.length} row(s) pending: ${state.dirty.map(ep=>`${ep.model_id||ep.slug} = ${uiState.endpointConcDrafts[ep.slug]}`).join(' · ')}`)}}
/* One number input per row. It is a plain type=number so the browser supplies the
increment/decrement stepper natively, matching scaleControls/buildSlotControls
rather than introducing a bespoke widget. */
function endpointConcInput(ep,maxReplicas,endpoints){const input=document.createElement('input');input.type='number';input.className='endpoint-conc-input';input.min='0';input.step='1';if(Number.isSafeInteger(maxReplicas)&&maxReplicas>=0)input.max=String(maxReplicas);input.value=endpointConcDraft(ep.slug,ep.concurrency);input.dataset.slug=ep.slug;input.setAttribute('aria-label',`${ep.model_id||ep.slug} target concurrency`);input.addEventListener('input',()=>{if(input.value===String(ep.concurrency??0))delete uiState.endpointConcDrafts[ep.slug];else uiState.endpointConcDrafts[ep.slug]=input.value;input.classList.toggle('dirty',uiState.endpointConcDrafts[ep.slug]!==undefined);renderEndpointApplyBar(endpoints,maxReplicas)});input.classList.toggle('dirty',uiState.endpointConcDrafts[ep.slug]!==undefined);return input}
/* Bulk apply: collect every dirty row into one request. The server validates the
whole batch before writing anything, so an out-of-range value rejects all rows
rather than half-applying. On a PARTIAL failure (a slug deleted underneath us)
the response carries applied[] and failed[] and ok=false -- both are surfaced by
name so the operator can never mistake a partial apply for a complete one. */
async function submitEndpointConcurrencies(endpoints,maxReplicas){const state=endpointApplyState(endpoints,maxReplicas);const feedback=el('endpoint-feedback');if(!state.capacityAvailable){feedback.className='bad';setText(feedback,'Cluster scaling capacity is unavailable; no change was made.');return}if(state.invalid.length){feedback.className='bad';setText(feedback,`Concurrency must be a whole number from 0 to ${maxReplicas}; no change was made.`);return}if(!state.dirty.length){feedback.className='muted';setText(feedback,'No pending concurrency changes.');return}const items=state.dirty.map(ep=>({slug:ep.slug,concurrency:Number(uiState.endpointConcDrafts[ep.slug])}));const payload=await postEndpoint('/api/generate/endpoints/scale-many',{items},`Applying ${items.length} concurrency change(s)…`,body=>{const applied=(body.applied||[]).map(row=>`${row.slug}=${row.concurrency}`).join(', ');const failed=(body.failed||[]).map(row=>`${row.slug} (${row.error})`).join(', ');return failed?`Applied ${(body.applied||[]).length}: ${applied||'none'} · FAILED ${(body.failed||[]).length}: ${failed}`:`Applied ${(body.applied||[]).length} concurrency change(s): ${applied}.`});if(!payload)return;/* Clear drafts only for rows the server confirmed; a failed row keeps its draft so the operator's intent is not silently lost. */(payload.applied||[]).forEach(row=>delete uiState.endpointConcDrafts[row.slug]);if((payload.failed||[]).length){feedback.className='bad'}}
...
function renderEndpoints(section,podCounts,maxReplicas){const body=el('endpoints');body.replaceChildren();const endpoints=section?.endpoints||[];if(!section?.available){const tr=document.createElement('tr');const td=document.createElement('td');td.colSpan=6;td.className='muted';setText(td,'Endpoint registry unavailable (schema not migrated).');tr.append(td);body.append(tr);renderEndpointApplyBar([],maxReplicas);return}if(!endpoints.length){const tr=document.createElement('tr');const td=document.createElement('td');td.colSpan=6;td.className='muted';setText(td,'No generate endpoints registered.');tr.append(td);body.append(tr);renderEndpointApplyBar([],maxReplicas);return}/* Drop drafts for endpoints that no longer exist so a deleted row cannot keep the Apply button armed forever. */Object.keys(uiState.endpointConcDrafts).forEach(slug=>{if(!endpoints.some(ep=>ep.slug===slug))delete uiState.endpointConcDrafts[slug]});endpoints.forEach(ep=>{const running=(podCounts&&podCounts[ep.slug])??ep.running??0;const tr=document.createElement('tr');const modelCell=document.createElement('td');setText(modelCell,ep.model_id||ep.slug);const hostCell=document.createElement('td');setText(hostCell,ep.host||ep.base_url||'—');const targetCell=document.createElement('td');const concCell=document.createElement('div');concCell.className='endpoint-conc-cell';concCell.append(endpointConcInput(ep,maxReplicas,endpoints));const runningLabel=document.createElement('span');runningLabel.className='endpoint-conc-running';setText(runningLabel,`/ ${running} running`);concCell.append(runningLabel);targetCell.append(concCell);const outcomeCell=document.createElement('td');const s=document.createElement('span');s.className='ok';setText(s,ep.recent_succeeded??0);const sep=document.createElement('span');setText(sep,' / ');const f=document.createElement('span');f.className='bad';setText(f,ep.recent_failed??0);outcomeCell.append(s,sep,f);const breakerCell=document.createElement('td');if(ep.breaker_open){breakerCell.className='endpoint-breaker-latched';const probe=ep.last_probe_status!=null?` · last HTTP ${ep.last_probe_status}`:'';setText(breakerCell,`LATCHED${ep.breaker_reason?` — ${ep.breaker_reason}`:''}${probe}`)}else{breakerCell.className='endpoint-breaker-ok';setText(breakerCell,ep.enabled?'OK':'disabled')}const actionsCell=document.createElement('td');const actions=document.createElement('div');actions.className='endpoint-actions';const editBtn=document.createElement('button');editBtn.type='button';setText(editBtn,'Edit API');editBtn.addEventListener('click',()=>submitEndpointEdit(ep.slug,ep.model_id));actions.append(editBtn);if(ep.breaker_open){const resetBtn=document.createElement('button');resetBtn.type='button';setText(resetBtn,'Reset');resetBtn.addEventListener('click',()=>submitEndpointReset(ep.slug));actions.append(resetBtn)}const deleteBtn=document.createElement('button');deleteBtn.type='button';deleteBtn.className='danger';setText(deleteBtn,'Delete');deleteBtn.addEventListener('click',()=>submitEndpointDelete(ep.slug,ep.model_id));actions.append(deleteBtn);actionsCell.append(actions);tr.append(modelCell,hostCell,targetCell,outcomeCell,breakerCell,actionsCell);body.append(tr)});renderEndpointApplyBar(endpoints,maxReplicas);const applyBtn=el('endpoint-apply');if(applyBtn)applyBtn.onclick=()=>submitEndpointConcurrencies(endpoints,maxReplicas);renderEndpointButtonsDisabled()}
function render(data){captureUiState();const took=uiState.lastPollMs==null?'':` · last fetch ${(uiState.lastPollMs/1000).toFixed(1)}s`;el('stamp').textContent=`Updated ${data.generated_at||'—'} · refreshes ${POLL_INTERVAL_MS/1000}s after each response${took}`; el('errors').replaceChildren();
 ...
 renderEndpoints(pg.generate_endpoints,k.generate_endpoint_pods||{},maxReplicas);
...
response and the start of the next, so it stays at the configured interval when
the server is fast and degrades gracefully (never overlapping) when it is slow.
The interval is injected from the server's own refresh cadence (default 15s, via
SWEGEN_DASHBOARD_REFRESH_SECONDS) so the browser can never poll faster than the
snapshot behind it actually changes. It was 5s, which against ~2,400 pods was by
itself enough to make the k3s API server fail its /livez probes. */
const POLL_INTERVAL_MS=__POLL_INTERVAL_MS__;let pollTimer=null;
    ...
    class Handler(BaseHTTPRequestHandler):
            ...
            if route == "/":
                body = (
                    ...
                    .replace(
                        ...
                        str(int(max(1.0, cache.refresh_seconds) * 1000)),
                    )
                    .encode()
                )
        ...
        def do_POST(self) -> None:
            ...
            try:
                ...
                return
            # Bulk scale carries one object per endpoint row, so it gets its own
            # (still bounded) ceiling; every other control route keeps the
            # original 1 KiB limit.
            body_limit = (
                MAX_BULK_SCALE_BODY_BYTES
                if self.path == "/api/generate/endpoints/scale-many"
                else MAX_CONTROL_BODY_BYTES
            )
            if not 0 < content_length <= body_limit:
                self._send_json(
                    400, {"error": f"request body must be between 1 and {body_limit} bytes"}
                )
                return
            try:
                ...
                elif self.path == "/api/generate/endpoints/scale-many":
                    response_fields = endpoint_registry.scale_many(
                ...
                return
            cache.refresh()
            # Key order is load-bearing: response_fields is spread AFTER the
            # default so a handler that reports its own "ok" wins. Bulk scale
            # uses this to return ok=False with its applied/failed lists on a
            # partial apply -- a 200 carrying the full per-row breakdown, rather
            # than a bare error that would hide which endpoints did take.
            self._send_json(
                200,
                {
                    "ok": True,
