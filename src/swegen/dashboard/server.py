...
.scale-controls{display:grid;grid-template-columns:90px 1fr;gap:4px;margin-top:6px}.scale-controls input,.scale-controls button{border:1px solid var(--line);border-radius:6px;padding:6px;background:#09182b;color:#edf4ff}.scale-controls button{cursor:pointer;background:#174b78}.scale-controls button:disabled{cursor:wait;opacity:.55}.scale-limit{grid-column:1/-1;color:var(--muted);font-size:11px}#scale-feedback{min-height:18px;margin-top:4px}
...
.node-details>summary{list-style-position:inside;cursor:pointer}.node-summary{display:grid;grid-template-columns:minmax(260px,1fr) minmax(240px,1fr) minmax(240px,1fr);gap:14px;align-items:center}.pod-list{margin:12px 0 2px 22px;display:grid;gap:5px}.pod-row{display:grid;grid-template-columns:minmax(300px,2fr) 110px 100px 80px;gap:10px;padding:5px 8px;border-left:2px solid var(--line);font:12px ui-monospace,SFMono-Regular,Consolas,monospace}.stage-counts{color:var(--muted);font-size:11px;margin-left:22px}@media(max-width:800px){.node-summary{grid-template-columns:1fr}.pod-row{grid-template-columns:1fr 1fr}.node-table-head{display:none}}
</style></head><body><main><h1>SWE-gen k3s + PGMQ</h1><div id="stamp" class="muted"></div><div id="errors"></div>
<h2>Stages</h2><div id="scale-feedback" class="muted"></div><div id="stages" class="grid"></div>
