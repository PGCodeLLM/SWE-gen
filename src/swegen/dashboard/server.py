...
const RANGE_HOURS_ALLOWED=[24,72,168];const uiState={chartScroll:{},expandedTasks:new Set(),expandedNodes:new Set(),resourceScroll:{left:0,top:0},scaleDrafts:{},scaling:false,buildSlotDrafts:{},buildSlotUpdating:false,endpointBusy:false,rangeHours:24,modelColors:{}};
...
function divergingModelTimeSeries(stageData,stage,rangeHours){const models=stageData?.models||[];const buckets=stageData?.buckets||[];const hours=Number.isFinite(rangeHours)?rangeHours:48;const scrollKey=`${stage}-model`;const wrap=document.createElement('div');wrap.className='stage-chart-wrap';const title=document.createElement('div');title.className='stage-chart-title';const label=document.createElement('span');setText(label,stage==='generate'?`15m outcomes by model · last ${hours}h`:`15m outcomes by generating model · last ${hours}h`);const legendNote=document.createElement('span');setText(legendNote,'success up · failed down');title.append(label,legendNote);wrap.append(title);if(!buckets.length||!models.length){const empty=document.createElement('div');empty.className='chart-empty';setText(empty,'No completed tasks');wrap.append(empty);return wrap}const bucketTotal=(bucket,key)=>models.reduce((sum,m)=>sum+((bucket.by_model?.[m]?.[key])||0),0);const anyRejected=buckets.some(bucket=>bucketTotal(bucket,'rejected')>0);const upMax=Math.max(1,...buckets.map(bucket=>bucketTotal(bucket,'succeeded')));const downMax=Math.max(1,...buckets.map(bucket=>bucketTotal(bucket,'failed')));const frame=document.createElement('div');frame.className='chart-frame';const yAxis=document.createElement('div');yAxis.className='chart-y-axis';[upMax,0,-downMax].forEach(value=>{const tick=document.createElement('span');setText(tick,value<0?`-${compactChartCount(-value)}`:compactChartCount(value));yAxis.append(tick)});const chart=document.createElement('div');chart.className='chart chart-diverging';chart.dataset.stage=scrollKey;chart._yAxis=yAxis;const cellData=[];chart._buckets=cellData;chart.addEventListener('scroll',()=>{if(chart.dataset.restoringScroll!=='true')uiState.chartScroll[scrollKey]=chartScrollSnapshot(chart);scheduleRescaleVisible(chart)},{passive:true});buckets.forEach(bucket=>{const up=bucketTotal(bucket,'succeeded'),down=bucketTotal(bucket,'failed'),rejected=bucketTotal(bucket,'rejected');/* Compact multi-line tooltip (rendered via white-space:pre) instead of the old wide middle-dot line that overflowed on hover. Header carries the up-total and labels the columns; one short line per active model. Rejected shows only when present, as a third success|failed|rejected column, so per-model lines stay short. */const showRej=rejected>0;const header=`total ${up} - success|failed${showRej?'|rejected':''}`;const activeModels=models.filter(m=>((bucket.by_model?.[m]?.succeeded)||0)||((bucket.by_model?.[m]?.failed)||0)||((bucket.by_model?.[m]?.rejected)||0));const modelLine=m=>{const c=bucket.by_model?.[m]||{};return `${m}: ${c.succeeded||0}|${c.failed||0}${showRej?`|${c.rejected||0}`:''}`};const tooltip=[compactChartTimestamp(bucket.t),header,...activeModels.map(modelLine)].join('\n');/* Rich tooltip nodes: the timestamp + header as plain lines, then one row per active model prefixed with its legend colour square (aria-label keeps the plain string for screen readers). */const tsLine=document.createElement('div');setText(tsLine,compactChartTimestamp(bucket.t));const headerLine=document.createElement('div');setText(headerLine,header);const tooltipNodes=[tsLine,headerLine];activeModels.forEach(m=>{const row=document.createElement('div');row.className='chart-tooltip-row';const sw=document.createElement('span');sw.className='chart-tooltip-swatch';sw.style.background=modelColor(m);const txt=document.createElement('span');setText(txt,modelLine(m));row.append(sw,txt);tooltipNodes.push(row)});const cell=document.createElement('div');cell.className='bucket';cell.tabIndex=0;cell.setAttribute('aria-label',tooltip);cell.addEventListener('mouseenter',event=>showChartTooltip(event,tooltip,tooltipNodes));cell.addEventListener('mousemove',positionChartTooltip);cell.addEventListener('mouseleave',hideChartTooltip);cell.addEventListener('focus',event=>showChartTooltip(event,tooltip,tooltipNodes));cell.addEventListener('blur',hideChartTooltip);const upSlot=document.createElement('div');upSlot.className='bar-slot bar-slot-up';const upBar=document.createElement('div');upBar.className='bar';upBar.style.height=`${Math.min(100,up/upMax*100)}%`;const upSegs=[];models.forEach(m=>{const value=(bucket.by_model?.[m]?.succeeded)||0;if(!value)return;const seg=document.createElement('div');seg.className='segment';seg.style.height=`${up?value/up*100:0}%`;seg.style.background=modelColor(m);upBar.append(seg);upSegs.push({seg,value})});upSlot.append(upBar);const downSlot=document.createElement('div');downSlot.className='bar-slot bar-slot-down';const downBar=document.createElement('div');downBar.className='bar bar-down';downBar.style.height=`${Math.min(100,down/downMax*100)}%`;const downSegs=[];models.forEach(m=>{const value=(bucket.by_model?.[m]?.failed)||0;if(!value)return;const seg=document.createElement('div');seg.className='segment';seg.style.height=`${down?value/down*100:0}%`;seg.style.background=modelColor(m);seg.style.opacity='0.55';downBar.append(seg);downSegs.push({seg,value})});downSlot.append(downBar);if(rejected>0){const marker=document.createElement('div');marker.className='rejected-marker';downSlot.append(marker)}const xTick=document.createElement('div');xTick.className='x-tick';const xLabel=document.createElement('span');xLabel.className='x-tick-label';setText(xLabel,compactChartTimestamp(bucket.t));xTick.append(xLabel);cell.append(upSlot,downSlot,xTick);chart.append(cell);cellData.push({cell,up,down,upBar,downBar,upSegs,downSegs})});frame.append(yAxis,chart);wrap.append(frame);const legend=document.createElement('div');legend.className='diverging-legend';models.forEach(m=>{const item=document.createElement('span');item.className='diverging-legend-item';const swatch=document.createElement('span');swatch.className='diverging-legend-swatch';swatch.style.background=modelColor(m);const text=document.createElement('span');setText(text,m);item.append(swatch,text);legend.append(item)});const note=document.createElement('span');note.className='diverging-legend-note';setText(note,anyRejected?'faded = failed (below axis) · ▏ = rejected':'faded = failed (below axis)');legend.append(note);wrap.append(legend);requestAnimationFrame(()=>{updateChartTicks(chart);rescaleVisible(chart)});if(typeof ResizeObserver!=='undefined'){const observer=new ResizeObserver(()=>{updateChartTicks(chart);rescaleVisible(chart)});observer.observe(chart);chart._tickObserver=observer}restoreChartScroll(chart,scrollKey);return wrap}
...
function validateQueueLine(label,queue){const line=document.createElement('div');line.innerHTML=`<b></b>: total <b></b> · ready <b></b> · leased/in-flight <b></b>`;const values=line.querySelectorAll('b');setText(values[0],label);setText(values[1],queue?.length??'—');setText(values[2],queue?.visible??'—');setText(values[3],queue?.in_flight??'—');return line}
...
async function poll(){try{const r=await fetch(`/api/pipeline/status?range_hours=${uiState.rangeHours}`,{cache:'no-store'});if(!r.ok)throw Error(`HTTP ${r.status}`);render(await r.json())}catch(e){el('stamp').textContent=`Dashboard fetch failed: ${e}`}}
el('endpoint-form').addEventListener('submit',event=>{event.preventDefault();submitEndpointRegister()});
{const rangeSelect=el('range-hours');if(rangeSelect){rangeSelect.value=String(uiState.rangeHours);rangeSelect.addEventListener('change',()=>{const value=Number(rangeSelect.value);uiState.rangeHours=RANGE_HOURS_ALLOWED.includes(value)?value:24;poll()})}}
