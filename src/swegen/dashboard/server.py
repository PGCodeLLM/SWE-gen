...
.chart-tooltip{position:fixed;z-index:1000;max-width:min(360px,calc(100vw - 16px));padding:5px 7px;border:1px solid #45658a;border-radius:6px;background:#06101d;color:#edf4ff;box-shadow:0 4px 16px #0009;font-size:11px;line-height:1.3;pointer-events:none;white-space:pre}.chart-tooltip[hidden]{display:none}
...
const modelColor=(model,models)=>{const index=models.indexOf(model);return index>=0?MODEL_PALETTE[index%MODEL_PALETTE.length]:MODEL_PALETTE[0]};
...
function divergingModelTimeSeries(stageData,stage){const models=stageData?.models||[];const buckets=stageData?.buckets||[];const scrollKey=`${stage}-model`;const wrap=document.createElement('div');wrap.className='stage-chart-wrap';const title=document.createElement('div');title.className='stage-chart-title';const label=document.createElement('span');setText(label,stage==='generate'?'15m outcomes by model · last 48h':'15m outcomes by generating model · last 48h');const legendNote=document.createElement('span');setText(legendNote,'success up · failed down');title.append(label,legendNote);wrap.append(title);if(!buckets.length||!models.length){const empty=document.createElement('div');empty.className='chart-empty';setText(empty,'No completed tasks');wrap.append(empty);return wrap}const bucketTotal=(bucket,key)=>models.reduce((sum,m)=>sum+((bucket.by_model?.[m]?.[key])||0),0);const anyRejected=buckets.some(bucket=>bucketTotal(bucket,'rejected')>0);const max=Math.max(1,...buckets.map(bucket=>Math.max(bucketTotal(bucket,'succeeded'),bucketTotal(bucket,'failed'))));const frame=document.createElement('div');frame.className='chart-frame';const yAxis=document.createElement('div');yAxis.className='chart-y-axis';[max,0,-max].forEach(value=>{const tick=document.createElement('span');setText(tick,value<0?`-${compactChartCount(-value)}`:compactChartCount(value));yAxis.append(tick)});const chart=document.createElement('div');chart.className='chart chart-diverging';chart.dataset.stage=scrollKey;chart.addEventListener('scroll',()=>{if(chart.dataset.restoringScroll!=='true')uiState.chartScroll[scrollKey]=chartScrollSnapshot(chart)},{passive:true});buckets.forEach(bucket=>{const up=bucketTotal(bucket,'succeeded'),down=bucketTotal(bucket,'failed'),rejected=bucketTotal(bucket,'rejected');/* Compact multi-line tooltip (rendered via white-space:pre) instead of the old wide middle-dot line that overflowed on hover. Header carries the up-total and labels the columns; one short line per active model. Rejected shows only when present, as a third success|failed|rejected column, so per-model lines stay short. */const showRej=rejected>0;const header=`total ${up} - success|failed${showRej?'|rejected':''}`;const rows=models.filter(m=>((bucket.by_model?.[m]?.succeeded)||0)||((bucket.by_model?.[m]?.failed)||0)||((bucket.by_model?.[m]?.rejected)||0)).map(m=>{const c=bucket.by_model?.[m]||{};return `${m}: ${c.succeeded||0}|${c.failed||0}${showRej?`|${c.rejected||0}`:''}`});const tooltip=[compactChartTimestamp(bucket.t),header,...rows].join('\n');const cell=document.createElement('div');cell.className='bucket';cell.tabIndex=0;cell.setAttribute('aria-label',tooltip);cell.addEventListener('mouseenter',event=>showChartTooltip(event,tooltip));cell.addEventListener('mousemove',positionChartTooltip);cell.addEventListener('mouseleave',hideChartTooltip);cell.addEventListener('focus',event=>showChartTooltip(event,tooltip));cell.addEventListener('blur',hideChartTooltip);const upSlot=document.createElement('div');upSlot.className='bar-slot bar-slot-up';const upBar=document.createElement('div');upBar.className='bar';upBar.style.height=`${Math.min(100,up/max*100)}%`;models.forEach(m=>{const value=(bucket.by_model?.[m]?.succeeded)||0;if(!value)return;const seg=document.createElement('div');seg.className='segment';seg.style.height=`${up?value/up*100:0}%`;seg.style.background=modelColor(m,models);upBar.append(seg)});upSlot.append(upBar);const downSlot=document.createElement('div');downSlot.className='bar-slot bar-slot-down';const downBar=document.createElement('div');downBar.className='bar bar-down';downBar.style.height=`${Math.min(100,down/max*100)}%`;models.forEach(m=>{const value=(bucket.by_model?.[m]?.failed)||0;if(!value)return;const seg=document.createElement('div');seg.className='segment';seg.style.height=`${down?value/down*100:0}%`;seg.style.background=modelColor(m,models);seg.style.opacity='0.55';downBar.append(seg)});downSlot.append(downBar);if(rejected>0){const marker=document.createElement('div');marker.className='rejected-marker';downSlot.append(marker)}const xTick=document.createElement('div');xTick.className='x-tick';const xLabel=document.createElement('span');xLabel.className='x-tick-label';setText(xLabel,compactChartTimestamp(bucket.t));xTick.append(xLabel);cell.append(upSlot,downSlot,xTick);chart.append(cell)});frame.append(yAxis,chart);wrap.append(frame);const legend=document.createElement('div');legend.className='diverging-legend';models.forEach(m=>{const item=document.createElement('span');item.className='diverging-legend-item';const swatch=document.createElement('span');swatch.className='diverging-legend-swatch';swatch.style.background=modelColor(m,models);const text=document.createElement('span');setText(text,m);item.append(swatch,text);legend.append(item)});const note=document.createElement('span');note.className='diverging-legend-note';setText(note,anyRejected?'faded = failed (below axis) · ▏ = rejected':'faded = failed (below axis)');legend.append(note);wrap.append(legend);requestAnimationFrame(()=>updateChartTicks(chart));if(typeof ResizeObserver!=='undefined'){const observer=new ResizeObserver(()=>updateChartTicks(chart));observer.observe(chart);chart._tickObserver=observer}restoreChartScroll(chart,scrollKey);return wrap}
