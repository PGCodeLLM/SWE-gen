...
class AnalyzeArgs:
    ...
    analysis_model: str = "claude-sonnet-5"  # Model for Claude Code classification
...
def _run_agent_trials(
    ...
    cmd = harbor_cmd_base() + [
        "run",
        *suffixed_docker_config_args(unique_parent, args.environment),
        "-p",
        str(dataset_path),
        "-t",
        task_id,
